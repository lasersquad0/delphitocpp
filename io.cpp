  ...
  parse_options:
	...
	switch(*options) { 
	  case '/':
		...
		const char *s = open_mode_decode[i].option;
		   ...
		   
{
	...
	if ((mode & (append_mode|rd_mode)) == (append_mode|rd_mode)) 
	    mode_str = (char*)"ab+";
	else if (mode & append_mode) mode_str = (char*)"ab";
	else if ((mode & rdwr_mode) == rd_mode) mode_str = (char*)"rb";
	else if ((mode & rdwr_mode) == wr_mode) mode_str = (char*)"wb";
	else if (mode & trunc_mode) mode_str = (char*)"wb+";
	else mode_str = (char*)"rb+";
    } else { 
	if ((mode & (append_mode|rd_mode)) == (append_mode|rd_mode)) 
	    mode_str = (char*)"a+";
	else if (mode & append_mode) mode_str = (char*)"a";
	else if ((mode & rdwr_mode) == rd_mode) mode_str = (char*)"r";
	else if ((mode & rdwr_mode) == wr_mode) mode_str = (char*)"w";
	else if (mode & trunc_mode) mode_str = (char*)"w+";
	else mode_str = (char*)"r+";
    } 
		   ...
		   integer* error_code)
{
    ...
    } else if (_stricmp(history, "old") == 0) {
	fd->f = fopen(name, "r+");
	mode = rdwr_mode;
	if (fd->f == NULL) { 
	    fd->f = fopen(name, "r");
    ...
    } else if (_stricmp(history, "unknown") == 0) {
	fd->f = fopen(name, "r+");
	mode = rdwr_mode;
	if (fd->f == NULL) { 
	    fd->f = fopen(name, "w+");
...
static void format_read(text_descriptor* td, const char* fmt, va_list ap) 
{ 
    td->desc.error = ioresult = IO_OK;
...
static void format_write(text_descriptor* td, const char* fmt, va_list ap) 
{
    td->desc.error = ioresult = IO_OK;
    while(*fmt != '\0' && td->desc.error == IO_OK) { 
	if (*fmt++ == '%') {
...
void cread(const char* fmt, ...) 
{
    va_list ap;
    va_start(ap, fmt);

    format_read((text_descriptor*)&input, fmt, ap);
...
void cwrite(const char* fmt, ...) 
{
    va_list ap;
    va_start(ap, fmt);

    format_write((text_descriptor*)&output, fmt, ap);
