...
деструктор почему то транслируетс€ неверно в obj->Destroy() вместо delete obj;


правильно парсить обь€вление TMyMetaClass = class of TMyClass

если у класса нету предка то добавл€ть наследование от TObject
...
* 
*в Delphi конструкторы наследуютс€ в C++ нет
*поэтому если в парент классе есть конструктор constructor Create(ui: integer);
 то в Delphi его можно вызвать дл€ child класса. ¬ —++ это не сработает.
 ѕодумать как быть с этим
